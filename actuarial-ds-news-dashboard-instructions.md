...
## Output structure
Two top-level sections: Actuarial, Data Science. Within each, list items with their tags shown (not separate subsections per tag combo — use tags as filter/sort controls). 5–8 items per section per day. Each item: headline (linked to source URL), short summary, practice area tag, business line tag (Actuarial only), item-type tag, source name, date.
